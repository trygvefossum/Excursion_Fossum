...
\reply: In the revised 'Closing remarks', we elaborate a bit more on space-time modeling including relevant references \cite{sigrist2015stochastic,richardson2017sparsity} and computational approximations required for three-dimensional grid models, on using more complicated criteria for the data collection, and thoughts on approaches for finite horizon optimization in our context with references to approximate dynamic programming \cite{Powell:11}.

In this context we brought up the issue of server communication for the AUV, which could be possible using wifi communication to a mother-ship at waypoints. First, this can enable more advanced modeling and computational routines. Also, this is a currently a hot topic in control engineering, and we believe there will be a need for statistical approaches for designing efficient sampling strategies for multiple robots in spatio-temporal domains. 
...
\reply: In the revised version we show EPs for myopic and look-ahead selection in this display. 
