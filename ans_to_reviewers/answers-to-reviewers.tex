...
In the revised manuscript these aspects are largely rewritten in the Introduction, and partly also in Section 2 on the expected integrated Bernoulli variance (EIBV) and in the Case Study ({\bf{TO DO 6}}). This change addresses some comments of AE, Rev #1 in particular.
...
In this sense, the revised version should provide ample motivation for statisticians.

In the revised manuscript these aspects are largely rewritten in the Introcuction and in Sect 2 and 3 on the statistical theory. Here, the main body contains the novel results for the expected uncertainty reduction in random sets exemplified by the EIBV, while the Appendix holds the proofs of key results. This change addresses the some comments of Editor, Associate Editor, Rec #2 and Rec #3.

\item We realized that a contribution of the current work is available code for practioners or statisticians who could try this themselves, or possibly extend it for their own cases. We have hence made a substantial effort in structuring {\bf{computer code for examples}} which are presented on a URL link to go with the paper. This open-source python code is also ready to run on more nuanced statistical models (non-stationary and non-isotropic). 
 
The simulation study section contains pointers to the associated open-source code. This change addresses the comments of Rev #1 in particular.
