...
\author{\fnms{Jo} \snm{Eidsvik}\thanksref{t3}, \ead[label=e2]{jo.eidsvik@math.ntnu.no}}
\author{\fnms{David} \snm{Ginsbourger}\thanksref{t4,t5}, \ead[label=e3]{david.ginsbourger@idiap.ch}}
\and
\author{\fnms{Kanna} \snm{Rajan}\thanksref{t2,t6,t7}. \ead[label=e4]{kanna.rajan@ntnu.no}}
...
\affiliation[t3]{Department of Mathematical Sciences, NTNU.}
\affiliation[t4]{Uncertainty Quantification and Optimal Design group, Idiap Research Institute, Martigny, Switzerland.}
\affiliation[t5]{Department of Mathematics and Statistics, Uncertainty of Bern, Switzerland.}
\affiliation[t6]{Department of Engineering Cybernetics, NTNU}
\affiliation[t7]{Underwater Systems and Technology Laboratory, Faculty of Engineering, University of Porto, Portugal.}
...
\printead{e3}}
\address{Kanna Rajan\\Department of Engineering Cybernetics\\ O. S. Bragstads Plass 2D, 7034 Trondheim\\ Norway\\
\printead{e4}}
...
\begin{abstract}
  ...
  combination of statistics and autonomous robotics provide new
  opportunities for experimental designs. In this work we develop
  methods for efficient spatial sampling applied to the mapping of coastal processes by providing informative descriptions of spatial
  characteristics of ocean phenomena. Specifically, we define a design
  criterion based on reducing uncertainty in the joint excursions of vector-valued Gaussian random fields, and derive tractable expressions for the expected
  Bernoulli variance reduction in such a framework. We demonstrate how these provide a description of
the boundary uncertainty in temperature and salinity variables, which is used to prioritize sampling efforts at locations that are ambiguous, making exploration more effective by building on myopic and
  non-myopic strategies. We use simulations to study the
  properties and differences between state-of-the-art approaches,
  followed by results from field deployments with an autonomous
  underwater vehicle as part of a case study mapping the boundary of a
  river plume. The results demonstrate the potential of combining statistical methods and robotic platforms to effectively inform and execute data-driven environmental sampling.
...
\item With limited available information about the state of the ocean, there is substantial value in reacting to
  ...
  sequentially.%\emph{in-situ}; this is usually referred to as the \emph{adaptivity gap} \citep{Krause2008phd}.
...
to design sampling strategies for for improving the estimation of joint ES with respect to temperature and salinity by reducing the spatially integrated
...
Our objective is to leverage statistical approaches to characterize a river
...
are available using methods developed by
...
\caption{Results from mapping the Nidelva river. \ref{fig:map} shows an
  overview of the survey area overlaid with the AUV paths in black and
  dashed line. The shaded region indicates the usual frontal
  region. \ref{fig:res_both} presents the collected temperature data
  as colored trails. Waypoint 5 (WP5) is where the
  two surveys deviate. \ref{fig:res1} and \ref{fig:res2} show the
  collected salinity data in the two surveys (as colored trails) overlaid on the final
  EPs, which indicate the AUVs statistical impression of the front in the surveys. For
...
01:00 PM, with a break in between. The resulting path of the
...
temperatures in the two surveys are shown as colored trails in Fig. \ref{fig:res_both},
...
estimated EPs for each survey in Fig. \ref{fig:res1} and
\ref{fig:res2}.
...
slightly between the two deployments, the resulting paths (after
waypoint 5) are shown to deviate. Survey 1 continued northwards,
