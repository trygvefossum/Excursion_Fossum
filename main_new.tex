...
height=0.26\textwidth,keepaspectratio]{Figures/real_myopic.png}\label{fig:real_myopic}}
\hspace{2em}
\subfigure[Route taken by myopic strategy.]{\includegraphics[
height=0.26\textwidth,keepaspectratio]{Figures/route_myopic.png}\label{fig:route_myopic}}
\hspace{2em}
\subfigure[Directional reduction of EBV.]{\includegraphics[
height=0.26\textwidth,keepaspectratio]{Figures/compass_myopic.png}\label{fig:compass_myopic}}
\caption{Pointwise Bernoulli variance reduction for two static and the myopic observation strategies. The prior EP is shown in
  \ref{fig:ebv_static_excu}. EBV reduction for each design shown in
  \ref{fig:ebv_static_north} and \ref{fig:ebv_static_east}. The true excursion for a realization of temperature and salinity is shown in \ref{fig:real_myopic}. The route taken by the myopic observation strategy is shown in \ref{fig:route_myopic}. The different alternative route choices and their expected EBV, illustrated as a compass from the perspective of the AUV, is shown in \ref{fig:compass_myopic}. Southward or $270 ^{\circ}$ is indicated by the thick and light color to be the best alternative direction.}
...
surveys for each one. An example of such a realization with a \textit{myopic} strategy is shown in Fig.~\ref{fig:route_myopic}. 
...
platforms.% , which statisticians can exploit to create general-purpose
% toolkits for similar applications.
...
% possibilities of wifi communication to a research vessel at waypoints,
% more advanced modeling and computational routines are possible, but
% with new challenges related to design and planning for when and where
% the communication should occur.
...
affiliation in an early version of this manuscript. The authors would
also like to thank Niklas Linde of the University of Lausanne for
providing constructive feedback about this work, and members of the
NTNU AURLab for help with AUV deployments

%  for pointing out the relevance of revisiting the proposed
% approach for estimating volumes of mixing zones, e.g. in hydrology.
