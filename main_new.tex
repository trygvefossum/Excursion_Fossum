...
\begin{aug}
\author{\fnms{Trygve Olav} \snm{Fossum}\thanksref{a}, \corref{} \ead[label=e1]{trygve.o.fossum@ntnu.no}}
\author{\fnms{Jo} \snm{Eidsvik}\thanksref{b}, \ead[label=e2]{jo.eidsvik@math.ntnu.no}}
\author{\fnms{David} \snm{Ginsbourger}\thanksref{c}, \ead[label=e3]{david.ginsbourger@idiap.ch}}
\and
\author{\fnms{Kanna} \snm{Rajan}\thanksref{d}. \ead[label=e4]{kanna.rajan@ntnu.no}}

\address{Department of Marine Technology\\ Trondheim, Norway\printead{e1}}
\affiliation[a]{Department of Marine Technology, The Norwegian University of Science and Technology (NTNU) \&\\ Centre for Autonomous Marine Operations and Systems (AMOS), Trondheim, Norway.}

\address{Department of Mathematical Sciences \printead{e2}}
\affiliation[b]{Department of Mathematical Sciences, NTNU.}

\address{Idiap \printead{e3}}
\affiliation[c]{Uncertainty Quantification and Optimal Design group, Idiap Research Institute, Martigny, Switzerland.}

\address{Department of Engineering Cybernetics,\printead{e4}}
\affiliation[d]{Department of Engineering Cybernetics, NTNU \&\\ Underwater Systems and Technology Laboratory, Faculty of Engineering, University of Porto, Portugal.}
...
\begin{abstract}
  ...
  design for effective characterization of oceanographic phenomena. We
  develop methods for efficient spatial sampling applied to the
  mapping of coastal processes by providing informative descriptions
  of spatial characteristics of ocean phenomena. Specifically, we
  define a design criterion based on reducing uncertainty in the joint
  excursions of random fields, and derive analytical expressions for
  the expected Bernoulli variance reduction in the excursion set of
  spatial temperature and salinity variables. We show how these can be
  applicable in static and sequential designs, building on myopic and
  look-ahead sampling strategies. We use simulations to study the
  properties and differences between state-of-the-art
  approaches. Results from field deployments with an autonomous
  underwater vehicle are also presented, for a case study mapping the
  boundary of a river plume. The results demonstrate the ability of
  the method to effectively inform and execute data-driven
...
made strides in advancing the state of sampling and consequently have
made robotics an integral part of ocean observation; our own work
sheds has contributed to this effort % driven by
...
an in-water operation time capacity depending on survey speed, payload
sensors and mission design. By deploying AUVs together with other
resources one can augment information obtained from either ocean
models, remote-sensing data, or fixed-location sensors on
buoys. Traditional operations and surveys with AUVs are usually
limited to observations along fixed transects on a spatial grid,
pre-programmed by the human operator. By using on-board algorithms to
continuously evaluate, update, and refine future sampling locations,
one can make information gathering adaptive and contextual to the
survey requirements \citep{das11b,fossuminformation,fossum18b}. The
space of sampling opportunities while still limited to the spatial
grid, allows for adaptation. For instance, an AUV could reconstruct or
...
river) and warmer saline waters of the fjord. Excursion sets (ES) and
excursion probabilities (EP) are useful starting points to measure the
ability to classify water masses. The goal of experimental sampling is
to improve this characterization, as quantified via changes in the
EPs.
...
to more than two variables is given in Section \ref{gen_k}.
...
\end{equation}
If the EPs in Eq. (\ref{eq:prob}) are close to $1$ or $0$ at a given
location, it is easy to classify water mass at this point as either
river or ocean; it is however challenging if the probability is close
to $0.5$.  The integrated Bernoulli variance (IBV) \cite{bect2019}, is
defined as:
...
use Gaussian modeling, which provides a practical and popular
probabilistic approach to modeling environmental data, allowing a
closed form evaluation of the IBVs, as derived in Section
\ref{sec:sur}.
...
specified by the mean vector and covariance matrix. For the
application of tracking river plumes, the model parameters are
specified from a short preliminary survey. The mean varies with
location, capturing the variability near the river mouth, while the
variance parameters and correlation between salinity and temperature
are assumed to be constant for all locations and within each water
mass.
...
which is not unrealistic for our case with a river plume as the decay
is related to the common water masses (Sect \ref{sec:case_study}). We
have:
...
otherwise. Moreover, we assume stationary isotropic random fields a
priori where the correlation function $\rho(\bx,\bx')$ solely depends
on $\bx$ and $\bx'$ via a Euclidean distance measure
...
possible design locations. It is also used when solving integrals
...
nodes in the grid. The conditional model for the data, given the true
temperature and salinity, is described by:
...
\right),
\end{equation}
with entries $m_{\cdot}$ and $a_{\cdot}$ extracted from the
conditional mean and covariance expressions in Eq. (\ref{gp_upd}).
...
Eq. (\ref{likelihood}). We next derive results that ease comparison of
several designs.
...
\caption{The commercially available Light Autonomous Underwater
  ...
  the experiment. Image Courtesy LSTS, Univ. of Porto, Portugal.}
\label{fig:lauv}
\end{figure} 
