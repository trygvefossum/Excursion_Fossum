...
\begin{abstract}
  ...
  expressions for the expected integrated Bernoulli variance reduction in such a
  ...
  autonomous underwater vehicle as part of a study mapping the
...
statistical properties of computations for efficient data
collection. We combine novel possibilities in marine
...
under the assumptions of GRF models, so it has limited flexibility for
...
\left(
\begin{matrix}
\tilde{\Sigma}_{u,u} & \tilde{\Sigma}_{u,v}\\
\tilde{\Sigma}_{v,u} & \tilde{\Sigma}_{v,v}
\end{matrix}\right)
\end{split}
\end{equation*}
with blocks
\begin{equation*}
\begin{split}
\tilde{\Sigma}_{u, v} &= \lambda_{[n+1,n+1]}(\uu)^T k_{[n]}(\bm{x},\bm{x}) \lambda_{[n+1,n+1]}(\vv)\\
\tilde{\Sigma}_{u, u} &= \lambda_{[n+1,n+1]}(\uu)^T k_{[n]}(\bm{x},\bm{x}) \lambda_{[n+1,n+1]}(\uu) + \futureCov{u, u}
\end{split}
\end{equation*}
...
We illustrate the expected Bernoulli variance (EBV) associated with different designs on a simple
...
We now turn to an example involving a full-fledged GRF. We consider a linear trend
\begin{align*}
...
We now study how the EBV [Eqn.\eqref{def:eibv}]
...
two-step look-ahead is to select the next observation location
...
applicability. The \textit{look-ahead} is slower, reaching levels that
are nearly impractical for execution on a vehicle. Some pruning of the
...
structures. Based on the initial runs we get a reasonable idea of the temperature and salinity of river and ocean waters, and also specify the mean by linear regression, where both temperature and salinity
were both assumed to increase linearly with the west location on the waypoint graph. Next, the residuals from the regression analysis were analyzed to specify the covariance parameters of the GRF model.
...
methods with robotic survey techniques for oceanographic
...
for real-time multivariable spatial data gathering and analysis on
...
uncertainties in random sets is extended with new results for the
expected integrated Bernoulli variance reduction achieved by spatial
sampling designs. This is provided in closed-form for the situation
...
likely approximate calculations by integrating terms only in the
...
models. If high-resolution ocean model or satelite data are available, it is hopefully possible to fit such more complex statistical models.
