...
indicator variables. Recent work on ESs and associated excursion
...
The AUVs considered here powered untethered platforms that operate at
$1$-$3$ m/s in the upper water column, where they are free to move in
six degrees of freedom (6 DOF). The in-water operation time capability
depends on survey speed, payload sensors and navigation; typically
this amounts to 8-48 hrs. AUVs typically use single-board computers
(SBCs), like the Raspberry Pi or a multicore GPU NVIDIA Jetson TX1
(quad-core 1.91 GHz 64-bit ARM machine, a 2-MB L2 shared cache, and 4
GB of 1600 MHz DRAM) for computation onboard.
...
\kc{Typically, I have been told in the past, it is bad form to start a section and
  then a subsection right away. I sort of see it, so, it might be good to
  provide a 1-2 sentence introduction here perhaps?}
...
set (typically in geostatistical studies, a subset of interest of the
ambient Euclidean space or of a sphere therein \kc{is this obvious to
  the readership why a sphere? It isn't for me certainly.}) and the
...
\citep{Molchanov2005,adler2009random}. %In what follows, we will express various quantities related to $\es$ in terms of $\gp$'s moments both for general and orthant $T$'s.
...
We let $d \in \mathcal{D}$ denote a sampling design, with $q$ sampling 
locations $\bu_d=\left(u_d^{(1)},\ldots, u_d^{(q)}\right)$. The
...
sampling locations, $\bG_d$ is the design matrix which in principle
...
additive error has independent elements, the covariance matrix $\bR_d$
...
predict $\gp[\bu]$, the multivariate Kriging equations holds with size
$p$ vector of conditional means \kc{This last sentence doesn't parse.
  Are they two separate statements?}:  
...
$\blambda(\x,\bu_d)=\bK^{-1}_y(\bu_d,\bu_d) \bk(\bu_d, \bu)$. Here,
the data covariance is
$K_y=\mbox{Cov}(\bY_d,\bY_d)=\bG_d\bK(\bu_d,\bu_d)\bG_d^T+\bR_d$ and
the cross-covariance is $\mbox{Cov}(\bY_d,Z(\x)) \bk(\bu_d, \x)=$,
\kc{There is a dangling '='. Is this legit?} assuming the measurement
error is independent of the GP.  The Kriging 
(cross-)covariance function can also be expressed in the same vein via
a $p \times p$ matrix 
\begin{equation}\label{eq:krig:cov}
\bSigma(\x,\x')=\bK(\x,\x')-\blambda(\x,\bu_d)^T \bK_y(\bu_d, \bu_d) \lambda(\x,\bu_d).
\end{equation}

Equations \eqref{eq:cokrig} and \eqref{eq:krig:cov} are used to
...
function of the data. This in turn means that the probabilities in
...
\eqref{eq:cokrig}, is in the form of $\ba+\bB \bV$, where
$\bV \sim N(0_q, \bK_y (\bu_d,\bu_d))$,
$\ba=(t_1,t_2)^t-\bmu(\bu)-\blambda(\bu,\bu_d)^T \bG_d \bmu(\bu_d)$
and $\bB=\blambda(\bu,\bu_d)^T$. The following then applies to give a
