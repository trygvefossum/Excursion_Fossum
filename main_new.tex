...
\begin{abstract}
  ...
  differences between state-of-the-art approaches followed by results
  from field deployments with an autonomous underwater vehicle as part
  of a case study mapping the boundary of a river plume. The results
...
\begin{itemize}
\item Full numerical ocean models cannot provide accurate results online if run on robotic sensing
  platforms, as on board computers cannot deliver the computational power required,
  hence statistical proxy models of the environment must be used instead.
...
The IBV \cite{bect2019}, is defined as: 
...
simulated case: mapping a freshwater plume defined by a
...
masses pulled to the east. The actual trends are, in practice, set
...
temperature is shown in Figures \ref{fig:true_temp} and
...
\caption{Results from mapping the Nidelva river} 
\label{fig:results}
\end{figure*}
...
The spatial-statistical design criterion, building on excursion sets, is relevant in our setting with different water mass properties; this could also be useful in other oceanographic settings, such as algae-blooms or frontal systems. Of course, other criteria that incorporate spatio-temporal effects could be included in this approach, making it especially relevant for more long-term decision-making.  For instance, managers and regulators must make difficult decisions related to fish farming, other marine resources operations, or environmental projects. Value of information analysis (VOI) \citep{Eidsvik:15} could be used in a similar vein as the integrated Bernoulli variance in the current paper, as both analytic methods explore which, when, and where information is likely to result in improved decision-making. 
