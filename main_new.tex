...
\title{Autonomous Oceanographic Data Collection via Uncertainty Reduction for  Excursion Sets of Vector-valued Random Fields} 
\runtitle{Autonomous Oceanographic Uncertainty Reduction}
...
% (AMOS).} Is the acronym needed now that this affiliation appears once only?
% Added NTNU, though. 
...
\end{frontmatter}
\section{Wannabe new introduction structure}
...
\begin{itemize}
    \item Oceanic data collection as a timely and crucial challenge 
    \item Uneasy to measure state variables varying continuously across space
    \item Some lend themselves well to random field modelling 
    \item Where to make measurements is a question related to experimental design
    \item Yet with AUVs measurements ought to be made along a trajectory 
    \item Furthermore, in several problems such as mapping Fjord versus ocean water, the goal is to reduce uncertainties on regions implicitly defined by several state variables such as temperature and salinity. 
    \item Besides this, AUV are able to revise their trajectory after intermediate measurements, calling for experimental design strategies that are adaptive and ideally also account for multiple steps ahead. 
    \item All this in the framework where several responses are modelled within a vector-valued random field framework and under observation noise calls for sophisticated design criteria and computational strategies for their optimization. 
    \item Here we aim to leverage recent progress in sequential design of computer experiments and adapt them to the specifics of guiding AUV measurements for parcimoniously distinguishing fjord from ocean waters. 
    \item While our problem involves multivariate observational responses to be observed along a continuous trajectory, our approach will take substantial advantage of existing work in the field of excursion set estimation mainly dedicated to computer experiments in the scalar-valued case, and where space exploration can be performed without constraints regarding the distance between successive design points. 
    \item Let us briefly review random field modelling and recent advances in sequential design of experiments before detailing further our proposed approach and outline.  
\end{itemize}

\subsection{Random field modelling and sequential design of experiments}
