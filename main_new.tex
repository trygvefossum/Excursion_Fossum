...
There are several obstacles when doing sampling with an AUV in the ocean, and we discuss some of these to motivate our setting, and also the modeling and methodological choices that will be made. First, full numerical ocean models based on complex physical processes cannot provide accurate results for online operations because the onboard computers cannot deliver the computational power required. Hence statistical proxy models of the environment must be used on the AUV. For our purpose, we rely on Gaussian process (GP) representations of the ocean variables of interest. Second, with limited available information about the state of the ocean, one must usually plan for a targeted operation where a relevant criterion is used to extract the most valuable designs, preferable in such a way that there is substantial value in reacting to information obtained from measurements taken in-situ. The acquired information must then be assimilated into the statistical model onboard the AUV, so that it can be used to re-compute the sampling criterion and in this way actively inform decisions on where to sample sequentially. We use a criterion focusing on the prediction of excursion sets (ESs) for multivariate processes of interest. Third, the number of sampling choices (i.e. locations, trajectories, and candidate
designs) is enormous, creating a trade-off between optimization (finding the most resource-efficient design to collect necessary data) and computability (arriving at a
solution in reasonable time). One must often compromise in both development and practical operations, and we will rely on short-horizon optimization routines among sequential designs that encapsulate nearest neighbors sampling in a waypoint graph created for the AUV operation.
...
We build on ESs and excursion
probabilities (EPs) for the spatial design of experiments. The goal is
...
where the underlying model is based on multivariate GPs. Another is that we embed this approach in a sequential strategy that runs onboard an AUV, and in applying this in the real-world
scenario of autonomously sampling temperature and salinity variables in a river plume.

Motivating examples relevant for ESs of multivariate processes are abundant. In medicine, doctors do not solely rely on a single symptom but must see several combined effects before making a diagnosis. In our context of spatial and environmental applications, a famous example is that of photosynthesis where plants and algae produce oxygen only when given enough of every component; light, carbon dioxide and water. Our key motivation in this paper is that temperature and salinity, combined, drive several processes in the oceans. For instance, salmon lice larvae are known to depend on relatively high temperature and high salinities to survive. Salmon lice are parasites to salmon, and the bivariate ES for temperature and salinity is hence important to understand for sustainable activities in salmon fisheries. The salinity and temperature excursions of a river plume similarly characterize its oceanographic and bio-geochemical processes \cite{hopkins2013detection,Pinto2018}, and sampling should be aimed at mapping relevant parts to reduce uncertainty in the temperature and salinity ES. 
...
\begin{figure}[!h] 
  ...
  \caption{\ref{fig:envir1} Traditional ocean observation based on
    ...
    numerous bio-geochemical processes. % and their interactions. 
...
but the locations of these features are highly uncertain, making deterministic planning
challenging. 
...
for fine resolution is by direct in-situ observation. 
...
At this scale of river plumes one can further often limit scope to covering only lateral near surface
spatial elements, and the case appears very amenable for AUVs usage. 
...
range of scientific payloads to survey the water column. The ones we considere here
operate at $\sim 1-3$ m/s, covering the upper water column, with an in-water operation time capacity dependent on survey speed, payload sensors and mission design. AUVs have limited computational capacity due to their limited energy and payload space. The computations is usually done on a single-board computer (SBC), like the Raspberry Pi or an multicore GPU NVIDIA Jetson TX1. 
...
along fixed transects on a spatial grid, called a waypoint graph, which is pre-programmed by the human operator. A more effective approach is to instead use onboard algorithms to continuously evaluate, update, and refine future sampling locations (sense-plan-act cycle in Fig. \ref{fig:envir1}), making the information
...
sampling opportunities is still limited by the waypoint graph, but adaptation
