...
\author{\fnms{David} \snm{Ginsbourger}\thanksref{t3}, \ead[label=e4]{david.ginsbourger@idiap.ch}}
\and
...
\kc{You seem to jump the gun in stating a challenge without
  connecting it to the sampling problem. Introduce a sentence as to
  why models (as a proxy) are important for \emph{understanding} ocean
  processes} First, full numerical ocean models based on complex
...
representations of the ocean variables of interest.\kc{Sorry....but
  this makes no sense whatsoever. What is the connection of you using
  a GP with sampling? You need to make a case first as to the
  viability of using GP in a marine robot first as a potential viable
  option, before you state why you want to use a GP}. Second, with
undersampling issues \kc{what issues? If there are issues, you state
  them first and then suggest solutions.} one must usually plan for
...
for sequential designs \kc{should this be, ``...so it has limited
...
dimensionality of the output $\no \geq 1$ is arbitrary \kc{Is the
  definition of such a 'p' and its dimensionality, obvious? This to me
  springs out of nowhere.}. Assume furthermore that the set of interest
...
characterization of uncertainties on such sets \kc{$\es$?} in the
...
salinity. \kc{What is the source of this data and this figure,
  i.e. how did you derive this plot?}
...
is a strong tendency to have concurrently low temperature and salinity
\kc{Same question as above. What is the basis for this correlation? Is
  this just an example or derived from actual data in the mixed layer
  in the front?}. The Bernoulli variance is similarly large for high
...
measurements alone. Both data are modeled by
$(Y_1,Y_2)^t=(Z_1,Z_2)^t+N(0,0.5^2I_2)$, while $Y_2=Z_2+N(0,0.5^2)$
when only temperature is measured \kc{I find this notation somewhat
  confusing. Earlier you had set up t1 and t2 to denote temp and
  salinity respectively. It might help to have \emph{some} notational
  similarity with that; otherwise it becomes mind bending I believe.}.
...
\kc{I feel this section above, which was present in the earlier
  submitted version too, but to which I did not comment, is
  significant as an example to the reader. But, as noted above, I
  don't understand the provenance of the data. It isn't made up,
  right?}
...
survey. If this design is selected, data $\by_{d^{j,s}}$ will be
gathered.  We further let
$\mathcal{Y}^{s-1} =\{(d^{j,1}, \by_{d^{j,1}}),\ldots, (d^{j,s},
\by_{d^{j,s}}) \}$ denote the set of all data gathered until stage
$s-1$ according to selected design $d^{j,1},\ldots,d^{j,s}$.  This
defines the AUV position at the stage $s$.  The data further defines
the conditional GP with mean
$\bbm^{s-1}(\x)=E(\gp[\x]|\mathcal{Y}^{s-1})$ from
Eq. (\ref{eq:cokrig}) and (cross) covariance matrix
$\Sigma^{s-1}=\mbox{Var}(\gp[\x]|\mathcal{Y}^{s-1})$ from
Eq. (\ref{eq:krig:cov}).

At the next stage, we choose where measurements $\by_{d^{j,s}}$ should
be gathered with design $d^{j,s}$. For the triangularized waypoint
graph that we use in our application, there are six possible
directions for $j$, and we must evaluate the design criterion for all
these.  The goal of the sampling design is to minimize the expected
uncertainty of random sets, and specifically the EIBV for our
...
closer to $0.5$ (see Table \ref{tab:sim_rhoab}, line \kc{row?}
...
calculation for EIBV is hence applicable in Eq.  \eqref{critSEQ}
\kc{Strangely in my TeX compilation this points to the earlier
  definition of $d^{*,s}$. Seems like some index is messed up in the
  sources; all indexes should perhaps be checked?} using the updated
GP model from stage $s-1$. Once the data is collected for the best
\kc{??}  design at stage $s$, the GP model is updated again. The mean
$\bbm^{s}$ and covariance matrix $\Sigma^{s}$ are used to compute the
next design at stage $s+1$, and so on.
...
optimal when one can gather only two more stages of
measurements. \kc{How can you be sure of this?} In addition to the
next stage of measurements $\by^{j,s}$, this look-ahead strategy,
anticipates the subsequent design indexed $j_2$ with data
...
that stage, implying that we have two levels of intermixed
minimizations and integrals.  We solve the first expectation by Monte
Carlo sampling of data $\by_{d^{j,s}}$ from its conditional
distribution. For each of these data samples, the second expectation
is solved using the closed-form expressions for EIBV from Section
\ref{sec:ESEP}.
