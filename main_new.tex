...
\begin{abstract}
  ...
  resources to understand dynamic processes in the water-column, the
  combination of statistics and autonomous robotics provide new
  opportunities for studying experimental design. In this work we develop
  methods for efficient spatial sampling applied to the mapping of
  coastal processes by providing informative descriptions of spatial
  characteristics of ocean phenomena. Specifically, we define a design
  criterion based on reducing uncertainty in the joint excursions of
  random fields, and derive analytical expressions for the expected
  Bernoulli variance reduction in the excursion set of spatial
  temperature and salinity variables, providing a description of
the boundary uncertainty that can be used prioritize sampling efforts at locations that are
ambiguous, making exploration more effective. We show how these can be
  applicable in static and sequential designs, building on myopic and
  non-myopic sampling strategies. We use simulations to study the
  ...
  river plume. The results demonstrate the potential of combining statistical methods and robotic platforms, and the abilities of the derived methods to effectively inform and execute data-driven environmental sampling.
...
based on closing the sensing and actuation loop. Myopic sampling, i.e. step-wise selection of
...
An adaptive AUV survey is split into many rounds (stages). The selection --on where to sample-- is made on the graph and defined by the nearest grid nodes in the domain $\mathcal{M}_g$.
...
and $\beta_{S,1}=29.0$ for the intercept at a reference map coordinate.
...
\kc{I don't understand what ``analytically available''
  means}. However, for the sequential approaches this is not the
...
the edge. \kc{Note there are plenty of language errors related to
  tense. Pl. do not mess with any more changes from here onwards.}
