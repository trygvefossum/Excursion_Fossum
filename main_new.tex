...
and variances to study the effect on the EP and expected Bernoulli variance.
...
\begin{table}[!t] \centering \caption{EP and Bernoulli variance for different correlations and variances (top rows), and expected Bernoulli variances for both temperature and salinity data, and only temperature data (bottom rows).}
...
expected Bernoulli variance results. This is presented for a design gathering
...
$\sigma_1$ and $\sigma_2$. The expected reduction of Bernoulli variance is
...
field (GRF). The class of GRF model we consider have a linear trend
\begin{align*}
...
(exponential, Mat\'{e}rn, etc.) and $\gamma$ defines the
...
specified. With extensive satellite data or prior knowledge from high-resolution ocean models, one could also possibly fit more complex multivariate
...
\end{align*}
and the spatial covariance has unit variance and a Mat\'{e}rn 3/2 kernel, $(1+\eta h)\exp (-\eta h)$, for Euclidean distance $h$ and decay parameter $\eta=0.5$.
...
Bernoulli variance reduction (at each grid point/nodes in the the waypoint graph) that would result from observing only one component of the field (temperature or salinity), or both at that point.
...
height=0.26\textwidth,keepaspectratio]{Figures/ebv_static_east.png}\label{fig:ebv_static_east}}
  \caption{Pointwise Bernoulli variance reduction for two different observation plans. The prior excursion probability is shown in \ref{fig:ebv_static_excu}. EBV reduction for each design shown in \ref{fig:ebv_static_north} and \ref{fig:ebv_static_east}.}
