...
\end{pmatrix},\nonumber
\end{equation}
where $\bS$ and $\bPsi$ are direct $K$-variate extensions of the
...
\begin{table}[!h] \centering \caption{EP and Bernoulli variance for
    ...
    Bernoulli variances for both data \kc{Do you mean an update of the
      data? What is 'both' in this context? T and S? I believe its the
    latter, and so should be explicit.} and only for
    temperature (bottom rows).}
...
gathering both data types $(y_a,y_b)$, and for a design with
temperature measurements $y_a$ alone. Having both data;
$(y_a,y_b)^t=(\xi_a,\xi_b)^t+N(0,0.5^2\bI)$, while
$y_a=\xi_a+N(0,0.5^2)$ when only temperature is measured.  The results
in Table \ref{tab:sim_rhoab} show that the expected Bernoulli variance
gets lower with larger standard deviations $\sigma_a$ and $\sigma_b$
(right columns) \kc{With just 2 data points isn't this a too strong
  statment to make?}. The reduction of Bernoulli variance is largest
...
from preliminary information or runs of oceanographic models \kc{a
  citation of such model use, e.g. the JFR paper might be in order
  here} solving differential equations for hydrodynamic flow. In this
example the mean is specified by $\beta_{a,1}=0.065$ and
$\beta_{b,1}=0.1$. Further, the intercepts \kc{Whats an intercept?} of
the temperature and salinity variables are set to $\beta_{a,0}=5.8$
and $\beta_{b,1}=29.0$.
...
basecase           &  35.21 &   \textbf{28.56} &       28.94 &         33.50 &        39.84 &          39.26 \\
    \bottomrule
    \end{tabular}}
  \caption{Simulation results for the final mean excursion set
    variance (Eq. \eqref{two_partsK}) for 20 replicates, 8
...
Two missions, Survey 1 and 2, was run successively from 11:00 AM to
01:00 PM, with a short break in between. The resulting path between
the selected waypoints are shown in spatial context in
...
separately overlaid with the estimated ES for each survey in
Fig. \ref{fig:res1} and \ref{fig:res2}.

Both Survey 1 and 2, successfully estimate and navigate the separation
zone while crossing the front multiple times. As conditions changed
...
2 turns west mapping the south-western part. The final prediction of
the front location given Fig. \ref{fig:res3} and \ref{fig:res4},
...
Survey 1 is greater than Survey 2, coming close to the survey area
borders in the south-western corner. A longer planning horizon would
