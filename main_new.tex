...
Others include \cite{wikle2013modern} who % present an overview of approaches for modeling
...
they define the gradients defining the frontal zone. At location
$\bx \in \mathcal{M} \subset \mathbf{R^2}$, we define the two process
variables by
...
% In the context illustrated by Figure \ref{fig:river4},
...
the observational effort of the AUV. A bivariate ES is defined via joint
variables \kcomment{'being below thresholds' -- is this correct?}, at
individual locations, for the process over $\mathcal{M}$. If $T_a$ is
the threshold for temperature and $T_b$ the threshold for salinity,
...
The actual calculation of the EP would require a model
specification. As we demonstrate below, Gaussian modeling assumptions
facilitate the computation.  If the EPs in Eq. (\ref{eq:prob}) are
close to $1$ or $0$, it is easy to classify water masses; it is
challenging if the probabilities are close to $0.5$. In such a case it
would be worthwhile to gather data to improve classification skill.

Recent work in statistical literature on ESs and EPs connected to
spatial statistics includes
\citep{french2013spatio,bolin2015excursion,french2016credible}.  Most
are based on GPs for spatial phenomena, as in our work.  Our own focus
is on ES as defined in Eq. (\ref{ES}) and location-wise EPs and the
uncertainty reduction achieved by informative sampling.  In this sense
our work is similar to \cite{chevalier2014fast} and
...
contribution of our work is to derive closed-form results for
expectations of ESs in the situation with multivariate processes.
...
We represent the bivariate random field of ocean temperature and
salinity by a Gaussian model.  At location $\bx \in \mathcal{M}$, the
...
\kcomment{Explain notion, even if this is a for a Statistics journal}
Here, the mean and standard deviation are allowed to vary with
location, which is important for capturing the variability such as
near a river mouth even as the correlation between salinity and
temperature is assumed to be uniform across all locations. While there
exist several multivariate spatial covariance functions
\citep{gneiting2010matern,genton2015cross}, in practice it has been
shown to be difficult to estimate all parameters from data. Typically
we assume the same correlation decay for salinity and temperature,
which is not unrealistic for a river plume. As a result, We use a
separable covariance function such that:
...
\kcomment{Ditto above; presumably x' is location separate from where an
  earlier measurement was taken?}

Moreover, we assume a stationary isotropic correlation function:

$\rho(\bx,\bx')=\rho(h)$ where h is the distance between $x$ and $x'$ 
($h=\sqrt{(\bx-\bx')^t (\bx-\bx')}$).

For implementation we discretize the spatial domain $\mathcal{M}$ on a
set of $n$ grid locations; $\mathcal{M}_g = \{\bx_i, i=1,\ldots,n \}$.
We let $\Delta$ be the area of each cell in the regular grid.  Using
vector notation, we collect the temperature and salinity variables on
the grid as:
...
where $\pi(\bxi)$ \kcomment{suggest you try a different (e.g. upper
  case) notation for $\pi$ given it is being used in different ways
  above} is the joint density function of $\bxi$, entries in length
$2 n$ vector $\bmu$ hold the expected values of temperature and
salinity at the grid locations, and the $2n \times 2n$ matrix
$\bSigma$ holds all the variance-covariances of temperature and
salinity variables at all grid locations.

\subsection{Updating Gaussian processes}

The GP mean and covariance in Eq. (\ref{prior} \kcomment{citation
  incorrect}) are updated when more information is available. In our
setting, the autonomous vehicle will collect a batch \kcomment{what is
  a 'batch' be explicit or use another term; 'iteration'?} of
temperature and salinity measurements along a survey path; after each
batch, the data is assimilated to get an updated model. We describe
one round of updating \kcomment{where? In this section?}. In Section
\ref{sec:heuristics} we discuss sequential data gathering and the
...
Regions with high variability in temperature or salinity % that surpasses the thresholds
% with a large margin (e.g. very high/low temperatures)
are not interesting, unless the uncertainty at these locations is very
large \kcomment{Huh? Not sure I understand. Are you saying the
  \emph{value} of a parameter if very large/small it is of less
  interest? Why?}. The objective is to explore regions where the
process variables are close to the thresholds \kcomment{You want to
  justify this some more
  -- why is it so? You can't make a categorical statement and leave it
  at that; the reviewers will want to know.}. With this mind, it is
valuable to explore locations $\bx$ where $p(\bx) \approx 0.5$. This
is what gives the maximum variance $p(\bx) (1-p(\bx))$ in the
indicator for the ES. \kcomment{This para is poorly structured. You
  should make a positive case for what you want to measure, justify
  and leave it as it is. Starting with what you \textbf{don't} want
  and then stating the positive is less convincing.}
...
The measure is dominated by locations with probabilities \kcomment{Of
  what?} close to $0.5$, or the integrated Bernoulli variance
\cite{Bect2019}.

Considering a single location, we explore the effect of different
correlation ($\gamma$) and prior variance. Fig. \ref{illus_bivarDens}
shows contour plots of three different densities where, the mean
temperature $T_a=5^o$ and mean salinity $T_b=30$ mg/l. The thresholds
are identical to these means. The variables have unit variance and
different correlation (low, medium and high). \kcomment{I don't
  understand these figures. Where did you get the three different
  correlations from? Add a note about how they were constructed
  and what is on the X/Y axis. Are the contours based on data from the
  table below?}

\begin{figure}[h!] \centering
  \includegraphics[width=0.99\textwidth]{Figures/illus_bivar.eps}
  \caption{Density contour plots having different correlations for
    temperature and salinity.}\label{illus_bivarDens} 
\end{figure}

Table \ref{tab:sim_rhoab} shows the EPs and the associated variances
in the ES (top rows). The results are computed for different
correlations $\gamma$, indicated in Fig. \ref{illus_bivarDens}, and
for standard deviations $1$ (left) and $2$ (right). The variance in
the ES is largest for high correlation, as there is more tendency of
joint small or high occurrences \kcomment{What is ``joint small''?}for
temperature and salinity in this case. EPs are the same for standard
deviation $1$ or $2$. \kcomment{Reformulate; this entire para is
  unclear and its connection to Fig. 3 above, is tenuous and vague. If
  there is a connection, then make it explicit.}


\begin{table}[!h] \centering \caption{EP and variance of ES for
    different correlations (top rows), and for data of both variables
