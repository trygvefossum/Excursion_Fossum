...
equal to $\widetilde{\bm{K}_{[0,n]}}^{-1} k(\bm{x}_{[n]}, \bm{x})$ with $\bm{x}_{[n]}=(\bm{x}_1,\dots, \bm{x}_n)$ and $\widetilde{\bm{K}_{[0,n]}}=k(\bm{x}_{[n]}, \bm{x}_{[n]})+\Delta_{[n]}$, assumed non-singular throughout the presentation, where $\Delta_{[n]}$ stands for the covariance matrix of Gaussian-distributed noise assumed to have affected measurements up to batch $n$. The co-kriging %(conditional) 
