...
\caption{Example run of the myopic strategy on a given realisation of the GRF model from \ref{sec:including_spatiality}. Past observation locations are depicted in green and current position of the AUV in cyan. The expected EBV reduction associated to data collection at neighbouring nodes of the current location is shown in \ref{fig:compass_myopic}. The thick and light color indicate the north-east node at $30 ^{\circ}$ to be the best possible choice.}
