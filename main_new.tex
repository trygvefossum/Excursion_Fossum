...
where $\lambda_{[n]}(\bm{x})$ is a vector of weights 
equal to $\left[ k(\bm{x}_{[n]}, \bm{x}_{[n]}) + R(\bm{x}_{[n]}, \bm{x}_{[n]}) \right]^{-1} k(\bm{x}_{[n]}, \bm{x})$ with matrix $R(\bm{x}_{[n]}, \bm{x}_{[n]})$ holding the observation noise covariance at the generalized locations. 
